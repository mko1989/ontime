...
export const useTimerMessageInputWithSettings = () => {
  const { text, visible } = useTimerMessageInput();
  const { data: settings } = useSettings();
  return { text, visible, overlay: settings.addMessageToTimer };
};
...
export const useMessagePreview = () => {
  ...
  return useRuntimeStore((state: RuntimeStore) => ({
    blink: state.message.timer.blink,
    blackout: state.message.timer.blackout,
    phase: state.timer.phase,
    secondarySource: state.message.timer.secondarySource,
    showTimerMessage: state.message.timer.visible && Boolean(state.message.timer.text),
    overlay: settings.data.addMessageToTimer,
...
export const setMessage = {
  ...
  timerOverlay: (payload: boolean) => sendSocket('message', { timer: { overlay: payload } }),
...
export const useGlobalCustomFields = createSelector((state: RuntimeStore) => state.globalCustomFields ?? {});
...
export const useTimerSocket = () => {
  ...
  return useRuntimeStore((state: RuntimeStore) => ({
    ...
    message: { ...state.message, timer: { ...state.message.timer, overlay: settings.data.addMessageToTimer } },
    time: state.timer,
    clock: state.clock,
...
export const useCountdownSocket = () => {
  const settings = useSettings();
  return useRuntimeStore((state: RuntimeStore) => ({
    playback: state.timer.playback,
    current: state.timer.current,
...
export const useBackstageSocket = () => {
  const settings = useSettings();
  return useRuntimeStore((state: RuntimeStore) => ({
    eventNext: state.eventNext,
    eventNow: state.eventNow,
...
export const useStudioTimersSocket = () => {
  const settings = useSettings();
  return useRuntimeStore((state: RuntimeStore) => ({
    eventNext: state.eventNext,
    eventNow: state.eventNow,
