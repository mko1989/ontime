...
export default function GeneralSettings() {
  ...
  const {
    ...
    defaultValues: data
      ? {
          ...data,
          addMessageToTimer: data.addMessageToTimer ?? false,
        }
      : {
          version: '4.0.0',
          serverPort: 4001,
  ...
  // Ensure addMessageToTimer is always set in form state
  const addMessageToTimerValue = watch('addMessageToTimer');
  useEffect(() => {
    if (addMessageToTimerValue === undefined) {
      setValue('addMessageToTimer', false, { shouldDirty: false });
    }
  }, [addMessageToTimerValue, setValue]);
        ...
        <Panel.Card>
          ...
          <Panel.Section>
            ...
            <Panel.ListGroup>
              ...
              <Panel.ListItem>
                <Panel.Field
                  title='Add Message to Timer'
                  description='When enabled, timer messages will be displayed alongside the timer instead of replacing it.'
                  error={errors.addMessageToTimer?.message}
                />
                <div>
                  <Checkbox
                    checked={Boolean(watch('addMessageToTimer'))}
                    onCheckedChange={(checked) => {
                      setValue('addMessageToTimer', Boolean(checked), { shouldDirty: true });
                    }}
                  />
                </div>
              </Panel.ListItem>
