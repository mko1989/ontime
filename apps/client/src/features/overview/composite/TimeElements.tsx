...
import {
  ...
  useTimer,
  useTimerMessageInputWithSettings,
  useTimerViewControl,
...
export function TimerOverview({ className }: { className?: string }) {
  ...
  if (visible && text && !overlay) {
    return (
      <TimeColumn
        label='Timer Message'
        value={text}
        state='active'
        className={cx([className, blink && 'blink'])}
      />
    );
  }
    ...
    return (
      <div style={{ display: 'flex', flexDirection: 'row', gap: '1rem', alignItems: 'center', width: '100%' }}>
        <div style={{ flexShrink: 0 }}>
          <TimeColumn label={title} value={display} state={timerState} className={className} />
        </div>
        <div
          className={cx([className, blink && 'blink'])}
          style={{
