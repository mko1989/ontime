...
function Backstage({ events, customFields, projectData, isMirrored, settings }: BackstageData) {
  ...
  const { eventNext, eventNow, rundown, selectedEventId, time, message } = useBackstageSocket();
              ...
              <div className='time-entry'>
                ...
                {message.timer.visible && message.timer.text && !message.timer.overlay ? (
                  <FitText
                    ...
                    min={16}
                    max={256}
                    className={cx(['time-entry__value', message.timer.blink && 'blink'])}
                  >
                    {message.timer.text}
                  </FitText>
                ) : (
                  <>
                    ...
                    {message.timer.visible && message.timer.text && message.timer.overlay && (
                      <div style={{ height: '0.6em', marginTop: '0.2em', color: 'var(--color-text-primary)' }}>
                        <FitText
                          mode='multi'
                          min={16}
                          max={256}
