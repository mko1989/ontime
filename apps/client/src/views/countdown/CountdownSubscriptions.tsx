...
export default function CountdownSubscriptions({ subscribedEvents, goToEditMode }: CountdownSubscriptionsProps) {
  ...
  if (message.timer.visible && message.timer.text && !message.timer.overlay) {
    return (
      <div className='list-container' style={{ display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <FitText
          mode='multi'
          min={32}
          max={256}
  ...
  return (
    <div className='list-container' onWheel={handleScroll} onTouchMove={handleScroll} ref={scrollRef}>
      {message.timer.visible && message.timer.text && message.timer.overlay && (
        <div
          style={{
            width: '100%',
            textAlign: 'center',
        ...
        >
          <FitText
            ...
            max={256}
            className={cx(['message', message.timer.blink && 'blink'])}
            style={{ width: '100%', textAlign: 'center', color: 'var(--color-text-primary)', height: '80px' }}
          >
            {message.timer.text}
          </FitText>
        </div>
