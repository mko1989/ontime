...
import * as globalCustomFieldsService from '../services/global-custom-fields-service/globalCustomFields.service.js';
 ...
 */
export function updateCustomFieldValue(eventId: string, fieldKey: string, value: unknown) {
  // Validate that the custom field exists in the project
  const customFields = getDataProvider().getCustomFields();
  if (!(fieldKey in customFields)) {
    throw new Error(`Custom field '${fieldKey}' not found. Please create the custom field first in settings.`);
  }

  // Get the current rundown to validate the event exists
  const { entries } = getCurrentRundown();
  const targetEntry = entries[eventId];
  if (!targetEntry) {
    throw new Error(`Event with ID '${eventId}' not found in rundown`);
  }

  // Use the existing change action logic to update the custom field
  const payload = {
    [eventId]: {
  ...
  return dispatchFromAdapter('change', payload, 'http');
}

/**
 * Updates a global custom field value (displayed in all rows)
 * @param fieldKey - The key of the custom field to update
 * @param value - The new value for the custom field (string or null/undefined to clear)
 * @returns Success response
 */
export function updateGlobalCustomFieldValue(fieldKey: string, value: unknown) {
  // Validate that the custom field exists in the project
  const customFields = getDataProvider().getCustomFields();
  if (!(fieldKey in customFields)) {
    throw new Error(`Custom field '${fieldKey}' not found. Please create the custom field first in settings.`);
  ...
  // Update the global custom field value
  globalCustomFieldsService.updateField(fieldKey, stringValue);
